...
void get_positives(
  ...
  if (verbose > 0) {
    ...
    // for (size_t i = 0; i < std::min(10LU, idx_len_vec.size()); ++i) { fprintf(stderr, "%lu, ", idx_len_vec[i]); }    //changed
    for (size_t i = 0; i < std::min(static_cast<size_t>(10), idx_len_vec.size()); ++i) { fprintf(stderr, "%lu, ", idx_len_vec[i]); }    //changed
    fprintf(stderr, "\n");
